...
const COUNTRY_TOOLTIP_MAP: Record<string, { name: string; flag: string; connections: string; community: string }> = {
  ...
  BO: { name: "Bolivia", flag: "🇧🇴", connections: "70", community: "0.6%" },
  EC: { name: "Ecuador", flag: "🇪🇨", connections: "58", community: "0.5%" },
  VE: { name: "Venezuela", flag: "🇻🇪", connections: "47", community: "0.4%" },
  PY: { name: "Paraguay", flag: "🇵🇾", connections: "35", community: "0.3%" },
  PA: { name: "Panamá", flag: "🇵🇦", connections: "24", community: "0.2%" },
  ES: { name: "España", flag: "🇪🇸", connections: "24", community: "0.2%" },
};
...
function PrelineRealJsVectorMap({ 
  ...
  countryDataMap?: Record<string, { name: string; flag: string; connections: string; community: string }>;
}) {
  ...
  useEffect(() => {
    ...
    mapInstanceRef.current = new jsVectorMap({
      ...
      onRegionTooltipShow(_event: any, tooltip: any, code: string) {
        const data = (countryDataMap && countryDataMap[code]) || COUNTRY_TOOLTIP_MAP[code];
  ...
  }, [activeCountryCodes, countryDataMap]);
...
function GeographyAnalyticsWidget({ isDark = false }: { isDark?: boolean }) {
  ...
  useEffect(() => {
    async function fetchLiveServerStats() {
      try {
        const res = await fetch("https://squadpanel-worker.latamcompanysquad.workers.dev/api/stats/geo-distribution");
        if (res.ok) {
          const data = await res.json();
          setStatsData({
            totalPlayers: data.totalUniquePlayers || 4925,
            newPlayers: data.newPlayers || 316,
            returningPlayers: data.recurringPlayers || 4609,
            avgEngagement: data.avgSessionTime || "55m",
            isLive: true,
          });
          ...
          if (Array.isArray(data.countries) && data.countries.length > 0) {
            ...
            const mappedTable = data.countries.map((c: any) => ({
              ...
              flag: flagMap[c.code] || "🌐",
              country: c.name,
              rawVisits: Number(c.connections) || 0,
              visits: (Number(c.connections) || 0).toLocaleString('es-ES'),
              purchases: `Jugadores: ${c.communityPercent || '0%'}`,
              change: c.trend ? c.trend.replace('+', '') : '1.0%',
              isPositive: !c.trend || !c.trend.includes('-')
            }));

            setLiveCountryTable(mappedTable);
          }
        }
      } catch (e) {
  ...
  const tooltipDataMap = useMemo(() => {
    const map: Record<string, { name: string; flag: string; connections: string; community: string }> = { ...COUNTRY_TOOLTIP_MAP };
                      ...
                      <div key={adm.name + idx} className="flex items-center justify-between px-2 py-1 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="flex flex-col truncate">
                            ...
                            <span className="text-[9px] text-slate-400 truncate">{handleName}</span>
                          </div>
                        </div>
